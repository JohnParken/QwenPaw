...
> **核心架构：共享 Harness API + PostgreSQL 持久任务表 + 常驻 Worker 池 + 多隔离执行槽 + 每次 Attempt 独立任务沙箱 + 云端工作区快照。**  
...
- **共享执行资源，但不共享可写用户运行环境。** Worker 可以服务多个用户，一个 Pod 可以并发运行多个隔离任务。
- **Runtime 在任务沙箱内，并保留受控网络。** 沙箱不是全面断网；模型、批准的 MCP、任务事件与取消通道必须可用。
...
| D06 | 一个执行槽同时运行一个 Attempt；执行期间创建独立任务沙箱 | 实施默认 |
| D07 | 不跨用户复用有状态 QwenPaw 解释器、浏览器 profile 或可写 HOME | 实施默认 |
| D08 | QwenPaw Runtime 位于任务沙箱内；高风险工具进一步缩小权限 | 实施默认 |
| D09 | Runtime 联网采用显式允许通道，工具权限不自动等于 Runtime 权限 | 实施默认 |
...
| D16 | 不把整个 Bucket、数据库、Worker 管理能力暴露给任务沙箱 | 实施默认 |
| D17 | 强制沙箱策略不允许被 Agent 配置、审批模式或工具参数关闭 | 实施默认 |
| D18 | 单 Pod 槽数、沙箱后端、网络实现、逐工具恢复范围须实测 | 待验证 |
...
| 独立 Python 进程就实现隔离 | 新进程只解决状态串用；还必须有真实文件、进程、资源和网络边界 |
| 开启 QwenPaw 原生 sandbox 就完成云端隔离 | 原生能力需核查与补强；新增不可绕过的任务级边界 |
| Runtime 在沙箱里就不能联网 | Runtime 保留允许的模型/MCP/控制通道，其他连接受限制 |
| 只给 Shell 加沙箱 | 整个 Runtime 先进入任务边界，再对高风险工具收紧 |
...
| Worker Supervisor | 常驻可信程序，领取任务、分配槽、管理沙箱、续租、清理 | 负责业务规划的 Agent |
| Slot / 执行槽 | 可同时承载一个 Attempt 的逻辑容量单位 | 天然安全边界 |
| Task Sandbox | 包围本次 Runtime 及其执行进程的系统级边界 | 单纯的工作目录或提示词 |
...
可信网页 → BFF → Harness 应用
                  ...
                  └── 内部 Worker API
                     ...
                     Worker Supervisor
                       ...
                       └── 多个 Slot → 独立 Attempt Sandbox
                                            └── QwenPaw Runtime / 受限工具
```
...
| Supervisor | 通过 File API 准备输入/工作副本、安全收集输出、管理沙箱 | 不持有存储 SDK 或底层存储凭据，不提供任意文件代理给 Agent |
| Runner | 在沙箱内运行 QwenPaw，使用已准备的本地内容 | 不管理平台文件服务，不领取其他任务 |
| SandboxManager | 建立并验证文件、进程、网络、资源限制 | 不等同于 File Service，不因后者拆分而改变 |
...
API 不必与 Worker 处于同一 K8s 集群。Worker 不暴露给网页，不需要 Kubernetes SDK 或创建工作负载权限。部署配置中的资源限制、只读根、非 root、禁止提权、seccomp 等是基础保护，不替代任务级沙箱。[S15]
...
| P1：强制执行边界 | 任务沙箱、工具收紧、干净环境、受控网络、资源约束和清理 | 越界测试失败、必要网络成功；不允许裸跑降级；同槽顺序执行不串数据 |
...
P1 通过前，不向不互信用户开放任意代码；P2 不等于已承诺故障续跑；至少通过 P3 才能作为具有恢复语义的长程 Harness。P4 的高密度配置不能放松 P1 的隔离门槛。
...
| T04 | 沙箱与宿主能力验证 | T01 | 候选后端的真实限制、网络及多槽兼容性有报告 |
...
| T06 | SandboxManager 生命周期 | T04、T05 | 创建/停止/销毁幂等，必需限制不可用则拒绝 |
| T07 | 文件、临时层和进程隔离 | T06 | 只挂当前工作区，临时目录独占，父进程资源不可见 |
| T08（P1/P4） | 每槽资源约束 | T04、T06 | P1 固定并实测单槽 CPU/内存/进程数/临时盘限制；P4 再优化容量与多槽密度；P0 macOS 缺失硬限制明确标 UNSUPPORTED |
| T09 | Runtime 网络与控制通道 | T06 | 模型/MCP/事件/取消可用，非允许目标不可达 |
| T10 | 工具级文件/网络收紧 | T03、T07、T09 | 高风险工具无路径绕过，不继承 Runtime 全部能力 |
...
| T27a（P0/P1） | 本地清理、槽隔离与安全复用 | T06/T07 的本阶段子集；不等待 T25 | P0 验证受控子进程、私有目录、凭据/FD 清理及失败停用；P1 验证实际生产沙箱清理；仅发送停止信号不算成功 |
...
| 沙箱后端 | 能否在目标 Worker 环境提供任务级、工具级、多槽隔离？ | P0/P1 |
| 宿主支持 | 命名空间、LSM、seccomp、资源委派是否满足？ | P0/P1 |
| 网络方案 | 允许服务、端口、DNS、IPC 与受控出口如何落地？ | P0/P1 |
...
| 风险 | 缓解 |
|---|---|
| 把普通子进程误认为沙箱 | 真实能力探测和系统级负向测试；限制不满足则失败 |
| 沙箱开启后 Runtime 断网 | 显式提供模型/MCP/控制/DNS 通道，并做正向测试 |
| Runtime 网络权限泄漏给工具 | 独立工具权限与强制出口，不能只设置代理变量 |
| 多槽资源互相拖垮 | 每 Attempt 限制、合理密度、独立 profile 与压测 |
...
安全保证针对声明的威胁模型与经过验证的策略，不覆盖一切未知内核/沙箱漏洞、平台管理员访问或外部服务失陷。不能承诺“绝对不会越界”；应提供实际配置、测试证据、补丁与运行监测。
...
| P1 Linux 环境 | 需要平台提供 OS/架构、内核/容器运行时/K8s 安全策略及测试访问；决定执行后端与受控出口是否真正可用 | P0-06/P1 验收前 | 不阻塞本机开发；阻塞生产隔离验收 |
...
4. P0—P3 一个常驻 Worker Pod 一个 Slot，P4 多槽另验；一个 Slot 同时执行一个 Attempt；每次新建 QwenPaw 进程与任务沙箱，不跨用户复用可写运行环境。
5. QwenPaw Runtime 在任务沙箱内；工具再收紧文件、网络和资源；不能只对 Shell 加提示词或路径检查。
6. Runtime 保留模型、批准 MCP 与本任务控制通道；工具不自动继承同等权限；网络和资源限制不可用时不裸跑降级。
...
11. 不把平台 DB/Bucket/Worker 管理凭据放进沙箱；Supervisor、Storage Broker、导入导出和内部协议本身也需最小能力和安全校验。
12. 优先采用新增适配层和少量上游补丁；不先全面云化 Hub，不强制引入工作流引擎。提交实现时同时交付隔离、恢复、网络和多槽测试。
...
**可交付的生产基线不是“QwenPaw 成功运行在 Pod 中”，而是：多用户不串域、沙箱边界真实生效、必要网络可用、任务状态持久、已提交边界可恢复、未知副作用可处理、单槽容量经过实测，多槽另过准入门槛。**
